...
function Base.propertynames(x::__JL_Ctag_1, private::Bool = false)
    (:c, :s, :i, if private
            fieldnames(typeof(x))
        else
            ()
...
function Base.getproperty(x::Ptr{libraw_tiff_tag}, f::Symbol)
    ...
    f === :val && return Ptr{__JL_Ctag_1}(x + 8)
    return getfield(x, f)
end
...
function libraw_open_wfile(arg1, arg2)
    ccall((:libraw_open_wfile, libraw), Cint, (Ptr{libraw_data_t}, Ptr{Cwchar_t}), arg1, arg2)
end
